...
from ymir.ymir_yolov5 import get_automl_config, get_weight_file


def start(cfg: edict) -> int:
    cfg = get_automl_config(cfg)
    logging.info(f'automl config: {cfg}')
...
def _run_training(cfg: edict) -> None:
    ...
    commands.extend([
        ...
        str(batch_size), '--data', data_yaml, '--project', project, '--cfg', f'models/{model}.yaml', '--name', name,
        '--weights', weights, '--img-size',
...
def _run_mining(cfg: edict) -> None:
    ...
    data_yaml = convert_ymir_to_yolov5(cfg)
    logging.info(f'generate {data_yaml}')
